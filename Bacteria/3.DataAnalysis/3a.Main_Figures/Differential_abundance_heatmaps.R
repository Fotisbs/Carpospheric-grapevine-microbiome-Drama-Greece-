...
psdt <- Drama_Terroir_Bacteria_Fruits
...
for(myotu in myotus){
  if(sum(dtcounts[,myotu]) == 0){
    ...
    mykrusk <- kruskal(dtcounts[,myotu], mydesign[rownames(dtcounts),]$Cultivar, group = T)
...
colnames(mat_fr_plot) <- mydesign$mynms
...
counts_fin_agg <- aggregate(counts_fin_t, by = list(mydesign$Cultivar), mean)
...
counts_fin_agg_relabund1 <- counts_fin_agg_relabund[,grep(paste(levels(mydesign$Cultivar),collapse = "|"),colnames(counts_fin_agg_relabund))]
# Remove not-needed columns
...
write.table(mat_fr_plot_agg, file = "Drama Fungi All D.A Heatmap.txt")
...
##Xana Range klimaka 01 Simantiko##
...
cairo_pdf(paste("Drama_Terroir_Fungi_All.ASVS.pdf", sep = ""), width =17, height =35)
