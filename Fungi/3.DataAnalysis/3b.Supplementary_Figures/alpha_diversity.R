...
adiv <- plot_richness(Drama_Terroir_Fungi_Fruits, measures=c("Shannon", "Fisher", "Observed", "ACE", "InvSimpson")) 
...
good <- MetaCommunity(t(Drama_Terroir_Fungi_Fruits@otu_table))$SampleCoverage.communities
...
alpha_wide_fact <- merge(alpha_wide, data.frame(sample_data(Drama_Terroir_Fungi_Fruits)), by = "row.names")
